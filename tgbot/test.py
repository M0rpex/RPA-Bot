...
def show_json(city, visa_id):
    with open(f'tgbot/list/{visa_id}/ofc/{city}_dict.json', 'r') as file:
        data = json.load(file)
    ...
    try:
        ...
        for day in schedule_days:
            ...
            file_path = f"tgbot/dates/{visa_id}/ofc/{city}_date.txt"
...
def show_json_cons(city, visa_id):
    with open(f'tgbot/list/{visa_id}/cons/{city}_dict.json', 'r') as file:
        data = json.load(file)
    ...
    try:
        ...
        for day in schedule_days:
            ...
            date = datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S")
            full_date = date.strftime("%d-%m-%Y")
            file_path = f"tgbot/dates/{visa_id}/cons/{city}_date.txt"
            with open(file_path, "w") as file:
                for date1 in schedule_days:
                    dn = date1.get('Date', '')
                    date = datetime.strptime(dn, "%Y-%m-%dT%H:%M:%S")
